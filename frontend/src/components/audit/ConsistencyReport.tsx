...
import { getAuditFlags, runConsistencyCheck, resolveFlag } from "@/api/client";
...
export function ConsistencyReport() {
  ...
  function loadFlags() {
    setLoading(true);
    getAuditFlags("consistency", false)
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h2>Consistency Flags ({flags.length})</h2>
        <button className="btn btn-sm" onClick={handleRun} disabled={running}>{running ? "Running..." : "Run Check"}</button>
      </div>
      <div className="card-body" style={{ padding: 0 }}>
        {loading ? <p className="text-sm text-muted" style={{ padding: 16 }}>Loading...</p> : flags.length === 0 ? (
          <div className="empty-state"><h3>No consistency flags</h3><p>Run a check to scan for inter-session discrepancies.</p></div>
        ) : (
              ...
              <tbody>
                {flags.map((f) => (
                  <tr key={f.id}>
                    ...
                    <td>
                      <button className="btn btn-sm" onClick={() => handleResolve(f.id)} disabled={resolving === f.id}>
